...
import { BundleManagementService, BundleItem } from '@/services/admin/bundleManagementService';
...
const EditBundleModal: React.FC<Props> = ({ bundle, onClose, onBundleUpdated }) => {
  const [bundleData, setBundleData] = useState({
    ...
    priority: bundle.priority || 1,
    min_items: bundle.min_items || 1,
    max_items: bundle.max_items || 1,
    start_date: bundle.start_date instanceof Date 
      ? bundle.start_date 
      : bundle.start_date?.toDate ? bundle.start_date.toDate() : new Date(bundle.start_date),
    end_date: bundle.end_date instanceof Date 
      ? bundle.end_date 
      : bundle.end_date?.toDate ? bundle.end_date.toDate() : new Date(bundle.end_date),
    image_url: bundle.image_url || '',
  });
  
  const [availableProducts, setAvailableProducts] = useState<Product[]>([]);
  const [selectedProducts, setSelectedProducts] = useState<BundleItem[]>(bundle.items || []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Update bundle
      await BundleManagementService.updateBundle(bundle.id, bundleData);
      
      // Remove all existing bundle items first
      if (bundle.items) {
        for (const item of bundle.items) {
          await BundleManagementService.removeProductFromBundle(bundle.id, item);
        }
      }
      
      // Add new products to bundle
      for (const product of selectedProducts) {
        await BundleManagementService.addProductToBundle(bundle.id, {
          ...product,
          id: `${bundle.id}-${product.product_id}`,
          bundle_id: bundle.id,
        });
      }
      ...
      setErrors(['Terjadi kesalahan saat mengupdate bundle']);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-md bg-white">
        <h2 className="text-2xl font-bold mb-4">Edit Bundle: {bundle.name}</h2>
